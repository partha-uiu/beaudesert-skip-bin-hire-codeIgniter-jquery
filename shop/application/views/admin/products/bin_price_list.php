        ...
        $('#dataTable .bulk-delete').each(function() {
            $(this).click(function(e) {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: '<?php echo site_url(CPREFIX . "/products/deleteBinPrice") ?>',
                    dataType: 'json',
...
<div class="row-fluid list_show <?php if(!empty($index) || empty($binprice)){ echo 'hidden'; } ?>">
    <div class="span12">
        <div class="box">
            <header>
                <div class="icons"><i class="icon-tasks" style="font-size: 20px; color: #0066cc;"></i></div>
                <h5 style="display: inline-block; font-size: 18px; font-weight: bold;">Zone List</h5>
            </header>
...
<!--                    <th> Product Price</th>-->
                    ...
                    <tbody>
                        ...
                        <?php foreach ($binprice as $item) { ?>
                        <?php // foreach ($productType as $type) { ?>
                        
                        
                            <tr>
                                <td class="center"><input class="check list-control-check" type="checkbox" value="<?php echo $item->b_id ?>" /></td>
                                        <!--<td><?php // echo $item->b_id; ?></td>-->
                                        <td><?php echo $item->p_type_title;?></td>
                                        <td><?php echo $item->p_title; ?></td>
                                        <td><?php echo $item->price; ?></td>
                                        <td><?php echo $item->product_zone; ?></td>                          
                             
                                <td class="action">
                                <a href="  <?php echo site_url(CPREFIX . "/products/bin_price/" . $item->b_id)?>"><i class="icon-pencil"></i> Edit</a>&nbsp;&nbsp;
